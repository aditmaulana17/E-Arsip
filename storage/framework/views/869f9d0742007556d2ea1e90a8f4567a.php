...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200/80 p-4">
        <form method="GET" action="<?php echo e(route('instansi.index')); ?>" class="flex flex-col sm:flex-row items-center gap-3">
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
                ...
                <?php if(request()->filled('search')): ?>
                    <a href="<?php echo e(route('instansi.index')); ?>" class="px-3 py-2 bg-slate-100 text-slate-600 rounded-lg text-sm font-medium hover:bg-slate-200 transition-colors border border-slate-200">
                        Reset
                    </a>
                <?php endif; ?>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php $__empty_1 = true; $__currentLoopData = $instansis; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        <td class="px-5 py-4">
                            <div class="font-semibold text-slate-800"><?php echo e($i->nama_instansi); ?></div>
                            <?php if($i->alamat): ?>
                                <div class="text-xs text-slate-400 truncate max-w-xs mt-0.5" title="<?php echo e($i->alamat); ?>"><?php echo e($i->alamat); ?></div>
                            <?php endif; ?>
                        </td>
                        <td class="px-5 py-4 whitespace-nowrap">
                            <span class="text-xs font-semibold px-2.5 py-1 rounded-full border <?php echo e(strtolower($i->jenis ?? '') == 'internal' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-blue-50 text-blue-700 border-blue-200'); ?>">
                                <?php echo e(ucfirst($i->jenis ?? 'Eksternal')); ?>

                            </span>
                        </td>
                        <td class="px-5 py-4 text-slate-600 whitespace-nowrap">
                            <?php echo e($i->kontak_person ?? '-'); ?>

                        </td>
                        <td class="px-5 py-4 text-slate-600 whitespace-nowrap">
                            <div><?php echo e($i->telepon ?? '-'); ?></div>
                            <?php if($i->email): ?>
                                <div class="text-xs text-slate-400"><?php echo e($i->email); ?></div>
                            <?php endif; ?>
                        </td>
                        <td class="px-5 py-4 text-right whitespace-nowrap">
                            <div class="inline-flex items-center gap-1">
                                <!-- Tombol Ubah Modal -->
                                <button type="button" 
                                        onclick="document.getElementById('modalEdit<?php echo e($i->id); ?>').classList.remove('hidden')" 
                                ...
                                <!-- Form Hapus Langsung -->
                              <form action="<?php echo e(route('instansi.destroy', $i)); ?>" method="POST" class="inline">
                                <?php echo csrf_field(); ?> 
                                <?php echo method_field('DELETE'); ?>
                                <button type="button" 
                                    onclick="confirmDelete(event, 'Data instansi <?php echo e($i->nama_instansi); ?> akan dihapus secara permanen!')" 
...
<div id="modalEdit<?php echo e($i->id); ?>" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 overflow-y-auto">
    <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
            ...
            <button type="button" onclick="document.getElementById('modalEdit<?php echo e($i->id); ?>').classList.add('hidden')" class="text-slate-400 hover:text-slate-600 p-1.5 rounded-lg hover:bg-slate-100">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
        <form method="POST" action="<?php echo e(route('instansi.update', $i)); ?>" class="p-6 space-y-4">
            <?php echo csrf_field(); ?> 
            ...
            <div>
                ...
                <input type="text" name="nama_instansi" value="<?php echo e($i->nama_instansi); ?>" required class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-blue-500">
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="jenis" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-blue-500">
                        <option value="eksternal" <?php echo e(($i->jenis ?? '') == 'eksternal' ? 'selected' : ''); ?>>Eksternal</option>
                        <option value="internal" <?php echo e(($i->jenis ?? '') == 'internal' ? 'selected' : ''); ?>>Internal</option>
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Kontak Person</label>
                    <input type="text" name="kontak_person" value="<?php echo e($i->kontak_person); ?>" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Telepon</label>
                    <input type="text" name="telepon" value="<?php echo e($i->telepon); ?>" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Email</label>
                    <input type="email" name="email" value="<?php echo e($i->email); ?>" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            <div>
                <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Alamat</label>
                <textarea name="alamat" rows="2" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-blue-500 resize-none"><?php echo e($i->alamat); ?></textarea>
            </div>
            <div class="flex justify-end gap-2 pt-2 border-t border-slate-100">
                <button type="button" onclick="document.getElementById('modalEdit<?php echo e($i->id); ?>').classList.add('hidden')" class="px-4 py-2 text-sm text-slate-600 hover:bg-slate-100 rounded-xl">Batal</button>
...
<div id="modalTambah" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 overflow-y-auto">
    <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
        ...
        <form method="POST" action="<?php echo e(route('instansi.store')); ?>" class="p-6 space-y-4">
            <?php echo csrf_field(); ?>
            <div>
                <label class="block text-xs font-semibold text-slate-700 uppercase mb-1">Nama Instansi <span class="text-rose-500">*</span></label>
