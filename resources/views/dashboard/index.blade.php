...
<div class="space-y-8">
    ...
    <div class="relative overflow-hidden bg-gradient-to-r from-blue-600 to-indigo-700 rounded-3xl p-6 sm:p-8 text-white shadow-xl shadow-blue-500/10">
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="space-y-2">
                ...
                <h1 class="text-2xl sm:text-3xl font-bold tracking-tight">Selamat Datang Kembali, {{ auth()->user()->name ?? 'Administrator' }}!</h1>
            ...
            <div class="flex items-center gap-3 shrink-0">
                <a href="{{ route('surat-masuk.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-white text-blue-700 hover:bg-blue-50 rounded-xl text-xs font-semibold shadow-sm transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                    <span>+ Surat Masuk</span>
                </a>
                <a href="{{ route('surat-keluar.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-500/30 hover:bg-blue-500/40 text-white border border-white/20 rounded-xl text-xs font-semibold backdrop-blur-md transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-sm hover:shadow-md transition group">
            ...
            <div class="mt-4 flex items-baseline justify-between">
                <span class="text-3xl font-extrabold text-slate-800 tracking-tight">{{ $totalSuratMasuk }}</span>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-sm hover:shadow-md transition group">
            ...
            <div class="mt-4 flex items-baseline justify-between">
                <span class="text-3xl font-extrabold text-slate-800 tracking-tight">{{ $totalSuratKeluar }}</span>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-sm hover:shadow-md transition group">
            ...
            <div class="mt-4 flex items-baseline justify-between">
                <span class="text-3xl font-extrabold text-slate-800 tracking-tight">{{ $suratPending }}</span>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-sm hover:shadow-md transition group">
            ...
            <div class="mt-4 flex items-baseline justify-between">
                <span class="text-3xl font-extrabold text-slate-800 tracking-tight">{{ $disposisiMenunggu }}</span>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm flex flex-col justify-between">
            <div>
                <div class="flex items-center justify-between pb-4 border-b border-slate-100">
                    ...
                    <a href="{{ route('disposisi.index') }}" class="text-xs font-semibold text-blue-600 hover:text-blue-700">Lihat Semua →</a>
                </div>

                <div class="mt-4 space-y-3">
                    @forelse($listDisposisi as $d)
                        <a href="{{ route('disposisi.show', $d->id) }}" class="block p-3.5 bg-slate-50 hover:bg-blue-50/50 rounded-xl border border-slate-100 transition space-y-1">
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-bold text-slate-700 truncate max-w-[180px]">{{ $d->suratMasuk->perihal ?? 'Surat Disposisi' }}</span>
                            ...
                            <p class="text-xs text-slate-500 line-clamp-1">Dari: {{ $d->dari->name ?? '-' }}</p>
                        </a>
                    @empty
            ...
            <div class="pt-4 mt-4 border-t border-slate-100">
                <a href="{{ route('disposisi.index') }}" class="w-full py-2.5 text-center block text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition">
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-sm">
        <div class="p-6 border-b border-slate-100 flex items-center justify-between">
            ...
            <a href="{{ route('surat-masuk.index') }}" class="px-3.5 py-1.5 text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition">
                Lihat Selengkapnya
            </a>
        </div>
                ...
                <tbody class="divide-y divide-slate-100 text-xs text-slate-700">
                    @forelse($suratMasukTerbaru as $sm)
                        <tr class="hover:bg-slate-50/60 transition">
                            <td class="py-4 px-6 font-bold text-blue-600 font-mono">{{ $sm->nomor_agenda ?? '-' }}</td>
                            <td class="py-4 px-6 font-semibold text-slate-800 max-w-xs truncate">{{ $sm->perihal ?? '-' }}</td>
                            <td class="py-4 px-6">{{ $sm->instansi->nama_instansi ?? '-' }}</td>
                            <td class="py-4 px-6">{{ $sm->kategori->nama_kategori ?? '-' }}</td>
                            <td class="py-4 px-6">
                                @php
                                    $statusClasses = [
                            ...
                            <td class="py-4 px-6 text-right">
                                <a href="{{ route('surat-masuk.show', $sm->id) }}" class="text-blue-600 hover:text-blue-800 font-semibold text-xs inline-flex items-center gap-1">
                                    Detail →
                                </a>
                            </td>
                        </tr>
