...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
            <!-- Edit Surat Keluar -->
            <a href="{{ route('surat-keluar.edit', $suratKeluar) }}" class="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold bg-amber-50 text-amber-700 hover:bg-amber-100 rounded-xl transition border border-amber-200 shadow-sm">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6 space-y-6">
        ...
        <div class="flex justify-between items-start pb-4 border-b border-slate-100 gap-4">
            <div>
                <h3 class="text-lg font-bold text-slate-800 leading-snug">{{ $suratKeluar->perihal }}</h3>
                <p class="text-xs text-slate-500 mt-1 flex items-center gap-2">
                    <span>No. Surat: <strong class="text-blue-600 font-semibold font-mono bg-blue-50 px-2 py-0.5 rounded border border-blue-100">{{ $suratKeluar->nomor_surat }}</strong></span>
                </p>
            </div>
        ...
        <dl class="grid grid-cols-1 md:grid-cols-2 gap-5 text-xs">
            <div>
                ...
                <dd class="mt-1.5 text-sm font-semibold text-slate-700 bg-slate-50 px-3.5 py-2.5 rounded-xl border border-slate-100">
                    {{ $suratKeluar->instansi->nama_instansi ?? $suratKeluar->tujuan ?? '-' }}
                </dd>
            </div>
            ...
            <div>
                ...
                <dd class="mt-1.5 text-sm font-semibold text-slate-700 bg-slate-50 px-3.5 py-2.5 rounded-xl border border-slate-100">
                    {{ $suratKeluar->kategori->nama_kategori ?? $suratKeluar->kategoriSurat->nama_kategori ?? '-' }}
                </dd>
            </div>
            ...
            <div>
                ...
                <dd class="mt-1.5 text-sm font-semibold text-slate-700 bg-slate-50 px-3.5 py-2.5 rounded-xl border border-slate-100">
                    {{ optional($suratKeluar->tanggal_surat)->format('d-m-Y') ?? (is_string($suratKeluar->tanggal_surat) ? $suratKeluar->tanggal_surat : '-') }}
                </dd>
            </div>
            ...
            <div>
                ...
                <dd class="mt-1.5 text-sm font-semibold text-slate-700 bg-slate-50 px-3.5 py-2.5 rounded-xl border border-slate-100">
                    {{ $suratKeluar->pembuat->name ?? $suratKeluar->user->name ?? '-' }}
                </dd>
            </div>
        ...
        <!-- Ringkasan / Isi Surat -->
        @if(!empty($suratKeluar->ringkasan))
        ...
        <div class="pt-4 border-t border-slate-100">
            ...
            @if(!empty($suratKeluar->lampiran_file))
                <a href="{{ route('lampiran.preview', $suratKeluar->lampiran_file) }}" target="_blank" class="inline-flex items-center gap-2 text-xs font-semibold bg-blue-50 text-blue-600 px-4 py-2.5 rounded-xl border border-blue-200/80 hover:bg-blue-100 transition shadow-sm">
