...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <!-- Export PDF -->
            <a href="{{ route('export.surat-keluar.pdf', request()->query()) }}" target="_blank"
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5">
        <form method="GET" action="{{ route('surat-keluar.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-center">
            ...
            <table class="w-full text-sm text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition duration-150">
                        <td class="px-5 py-4 font-semibold text-blue-600 whitespace-nowrap">{{ $s->nomor_surat }}</td>
                        <td class="px-5 py-4 text-slate-500 whitespace-nowrap">
                            {{ optional($s->tanggal_surat)->format('d/m/Y') ?? '-' }}
                        </td>
                        ...
                        <td class="px-5 py-4 whitespace-nowrap">
                            ...
                            <span class="inline-block px-2.5 py-1 rounded-full text-xs font-semibold border {{ $badgeClass }}">
                                {{ ucfirst($s->status ?? 'Draf') }}
                            </span>
                        </td>
                        <td class="px-5 py-4 text-center whitespace-nowrap">
                            <div class="inline-flex items-center gap-1">
                                <a href="{{ route('surat-keluar.show', $s) }}" class="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Lihat Detail">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <a href="{{ route('surat-keluar.edit', $s) }}" class="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition" title="Ubah Data">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                <form action="{{ route('surat-keluar.destroy', $s) }}" method="POST" class="inline">
                                    @csrf 
                                    @method('DELETE')
                                    <button type="button" 
                                            onclick="confirmDelete(event, 'Surat Keluar {{ $s->nomor_surat }} akan dihapus secara permanen!')" 
