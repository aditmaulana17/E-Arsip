...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500">Kelola dan pantau seluruh arsip surat masuk organisasi Anda.</p>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <!-- Export PDF -->
            <a href="{{ route('export.surat-masuk.pdf', request()->query()) }}" 
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                <thead class="bg-slate-50 text-slate-600 font-semibold uppercase text-xs tracking-wider border-b border-slate-200">
                    <tr>
                        <th class="px-4 py-3.5">No. Agenda</th>
                        <th class="px-4 py-3.5">Nomor Surat</th>
                        <th class="px-4 py-3.5">Tgl Terima</th>
                        <th class="px-4 py-3.5">Instansi</th>
                ...
                <tbody class="divide-y divide-slate-200">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        <td class="px-4 py-3 font-semibold text-brand-600 whitespace-nowrap">#{{ $s->nomor_agenda }}</td>
                        <td class="px-4 py-3 font-medium text-slate-800 whitespace-nowrap">{{ $s->nomor_surat }}</td>
                        <td class="px-4 py-3 text-slate-600 whitespace-nowrap">{{ $s->tanggal_terima ? $s->tanggal_terima->format('d/m/Y') : '-' }}</td>
                        ...
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            <div class="inline-flex items-center gap-1.5">
                                <a href="{{ route('surat-masuk.show', $s) }}" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Lihat Detail">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <a href="{{ route('surat-masuk.edit', $s) }}" class="p-1.5 text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Ubah Data">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                <a href="{{ route('disposisi.create', $s) }}" class="p-1.5 text-purple-600 hover:bg-purple-50 rounded-lg transition-colors" title="Buat Disposisi">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>
                                </a>
                                <form action="{{ route('surat-masuk.destroy', $s) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus surat ini?')">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="8" class="text-center py-12">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-slate-600 font-medium">Belum ada data surat masuk</p>
